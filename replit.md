...
- **Public Supply Sites Map**: Interactive public map displaying food hubs and supply distribution centers with inventory recency heat mapping. Created in partnership with Buncombe County HHS and WNC Food Coalition for SNAP funding gap support. Features: (1) Leaflet-based mapping with color-coded markers (green=recently updated, yellow=moderate, red=needs update, gray=no data), (2) Publicly accessible without authentication at /supply-sites-map, (3) Embeddable for partner organizations via ?embed=true query parameter, (4) Configurable thresholds via URL parameters (?green=7&yellow=30), (5) Clickable site popups displaying name, address, hours, site type, accepting/distributing status, and last inventory update date, (6) Mobile-optimized responsive design, (7) Privacy-aware filtering showing only PUBLIC sites from Airtable (respects isPubliclyVisible flag). Airtable integration fetches Site and Site Inventory tables to calculate inventory recency. Backend API at /api/supply-sites/public with server-side filtering and threshold calculation.
