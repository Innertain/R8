...
## System Architecture
### UI/UX Decisions
The platform features a clean, professional design with a focus on readability. It utilizes a soft color scheme with light grey backgrounds and improved contrast. Interactive UI elements incorporate subtle hover effects, such as slight lifting, enhanced shadows, and blue border highlights for clear visual feedback. The design is fully mobile-optimized with comprehensive responsive layouts including: mobile-first grid breakpoints (1 col mobile → 2 col small → 3 col large → 6 col extra-large), balanced mobile spacing optimizations (60vh hero, 35vh mission on mobile), moderate negative margin overlapping (-2rem to -3rem) for smooth section transitions, partner card text overflow prevention with flex centering and optimized sizing, and complete CSS media query overrides for seamless mobile experience across all screen sizes. Navigation uses icon-based tabs with responsive layouts (vertical on mobile, horizontal on desktop).
