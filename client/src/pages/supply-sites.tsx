...
export default function SupplySitesPage() {
  // Check for auto-onboarding URL parameter
  const urlParams = new URLSearchParams(window.location.search);
  const autoOnboard = urlParams.get('onboard') === 'true';
  
  const [showOnboarding, setShowOnboarding] = useState(autoOnboard);
    ...
    <Card data-testid="card-welcome-step">
      ...
      <CardContent className="space-y-6">
        ...
        <ul className="space-y-2">
          ...
          <li className="flex items-start gap-2">
            <CheckCircle className="h-5 w-5 text-green-500 mt-0.5" />
            <span>Connect to the R8 mutual aid network</span>
          </li>
        </ul>
